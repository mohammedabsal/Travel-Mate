...
import { ArrowRight, Calendar, Clock3, MapPinned, Route, Wallet, Sparkles } from 'lucide-react';
...
export function DashboardOverview() {
  ...
  return (
    <div className="space-y-7">
      <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="rounded-[2rem] border border-white/60 bg-white/80 p-5 shadow-xl shadow-violet-500/5 backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/60 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-violet-700 dark:text-violet-300">Overview</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-4xl">Your travel command center</h2>
            <p className="mt-2 text-slate-600 dark:text-slate-300">Monitor active itineraries, budget confidence, and day-level execution in one motion-driven dashboard.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge className="rounded-full border-0 bg-emerald-500/15 text-emerald-700 dark:text-emerald-300">Live sync on</Badge>
            <Badge className="rounded-full border-0 bg-violet-500/15 text-violet-700 dark:text-violet-300">AI recommendations</Badge>
          </div>
        </div>
      </motion.section>
